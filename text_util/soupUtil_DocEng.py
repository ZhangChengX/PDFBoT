...
def getStartLocateIndexFromLine(line, soup, sentence):
    index=0
    startIndex=[]
    for i, nodeID in enumerate(line):
        ...
        listOfNavStrObject = getNavigableStringObject(soup[str(nodeID)],[])
...
def getEndingLocateIndexFromLine(line, soup, sentence):
    ...
    for i, nodeID in enumerate(line):
        ...
        print('soup test = ' + str(soup[str(nodeID)]))
        # listOfNavStrObject = getNavigableStringObject(soup.find('div', {"data-page-no": pageNo}).select(selectStr)[0], [])
        listOfNavStrObject = getNavigableStringObject(soup[str(nodeID)],[])
...
def getColourdDivSoup(tagStr, colourStr):
    ...
    newSoup.insert(0, colourSoup)
    return str(newSoup)
def ColourDivInSoup(soupDict,nodeID, colourStr):

    tagStr = str(soupDict[nodeID])
    # print('tag str = '+str(tagStr) )
    ...
    newSoup.insert(0, colourSoup)
    soupDict[nodeID].replace_with(newSoup)
    soupDict[nodeID]=newSoup
